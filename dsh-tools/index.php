<?php include "./includes/header.html" ?>
<?php include "./includes/sidebar.html" ?>

<section class="pink-bg wrapper"> 
  <div class="container-fluid" style="padding:20px;
  height: 220px;">
      <span class="title">Quick Analytics</span>
      <span class="right-content"><input style="border: none;  border-radius: 3px;padding: 2px;" type="date"></span>
  
      <div class="wrapper" style="padding-top: 10px;"> 
        <div class="column-3" style="border-left:6px solid #a351be;">
          <div class="left-content"> 
          <span style="font-size:13px">TOTAL CHECK OUTS</span><br>
          <span><h1 style="font-weight: 700;">99</h1></span>
          </div>
        </div>
        <div class="column-3" style="border-left:6px solid #3cc9e2;">
          <div class="left-content"> 
          <span style="font-size:13px">TOTAL SPENT</span><br>
          <span><h1 style="font-weight: 700;">$9,999.99</h1></span>
          </div>
        </div>
      </div>
  </div>
</section>
<section class="main-section">
  <div class="container-fluid db-search">
    <div class="row">
      <div class="col-md-6">
        <input type="text" placeholder="Search">
      </div>
      <div class="col-md-6" style="display:flex; align-items:center; justify-content:flex-end;">
        <a href="#"><button class="login-btn">Export Data</button></a>
      </div>
    </div>
  </div>
  <div class="container-fluid db-padding">
    <table width="100%" style="border-radius: 5px;background-color: #fff;border-radius: 10px;font-size: 12px;">
      <thead style="background-color: #f0f4f7;">
        <th scope="col">PRODUCT</th>
        <th scope="col">PRODUCT NAME</th>
        <th scope="col">COLOR</th>
        <th scope="col">SIZE</th>
        <th scope="col">DATE / TIME</th>
      </thead>
      <tbody>
        <tr>
          <td><img src="./assets/images/product.png" alt=""></td>
          <td>Kobe 6 Protro All-Star</td>
          <td>RED</td>
          <td>8</td>
          <td>2021-12-05  /  15:23</td>
        </tr>
        <tr>
          <td><img src="./assets/images/product.png" alt=""></td>
          <td>Kobe 6 Protro All-Star</td>
          <td>RED</td>
          <td>0</td>
          <td>2021-12-05  /  15:23</td>
        </tr>
        <tr>
          <td><img src="./assets/images/product.png" alt=""></td>
          <td>Kobe 6 Protro All-Star</td>
          <td>RED</td>
          <td>10</td>
          <td>2021-12-05  /  15:23</td>
        </tr>
        <tr>
          <td><img src="./assets/images/product.png" alt=""></td>
          <td>Kobe 6 Protro All-Star</td>
          <td>RED</td>
          <td>11</td>
          <td>2021-12-05  /  15:23</td>
        </tr>
        <tr>
          <td><img src="./assets/images/product.png" alt=""></td>
          <td>Kobe 6 Protro All-Star</td>
          <td>RED</td>
          <td>8</td>
          <td>2021-12-05  /  15:23</td>
        </tr>
        <tr>
          <td><img src="./assets/images/product.png" alt=""></td>
          <td>Kobe 6 Protro All-Star</td>
          <td>RED</td>
          <td>9</td>
          <td>2021-12-05  /  15:23</td>
        </tr>
        <tr>
          <td><img src="./assets/images/product.png" alt=""></td>
          <td>Kobe 6 Protro All-Star</td>
          <td>RED</td>
          <td>10</td>
          <td>2021-12-05  /  15:23</td>
        </tr>
        <tr>
          <td><img src="./assets/images/product.png" alt=""></td>
          <td>Kobe 6 Protro All-Star</td>
          <td>RED</td>
          <td>11</td>
          <td>2021-12-05  /  15:23</td>
        </tr>
        <tr>
          <td><img src="./assets/images/product.png" alt=""></td>
          <td>Kobe 6 Protro All-Star</td>
          <td>RED</td>
          <td>8</td>
          <td>2021-12-05  /  15:23</td>
        </tr>
        <tr>
          <td><img src="./assets/images/product.png" alt=""></td>
          <td>Kobe 6 Protro All-Star</td>
          <td>RED</td>
          <td>98</td>
          <td>2021-12-05  /  15:23</td>
        </tr>
      </tbody>
    </table>
  </div>
</section>
<?php include "./includes/footer.html" ?>